...
public class PostgresqlTestContainersExtension extends GenericContainer implements BeforeAllCallback {
    ...
    public void start() {
        super.start();
        String jdbcUrl = String.format("jdbc:postgresql://%s:%s/test?socketTimeout=300",
                DockerClientFactory.instance().dockerHostIpAddress(), container.getMappedPort(5432));
        ...
        log.info("Started PostgreSQL container with JDBC URL: {}", jdbcUrl);
    }
